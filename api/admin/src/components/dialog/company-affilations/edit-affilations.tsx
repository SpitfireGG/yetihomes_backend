...
import { ConpanyTeamDataForm } from "@/@types/company";
import { convertTitleIntoSlug } from "@/utils/common";
import { CompanyAffiliationApiResponse, CreateAffiliationDto } from "@/@types/company/affiliation";
...
export const EditCompanyAffilationsDialog = ({ affilationId, data }: Props) => {
    const { data: languages } = useLanguages();
    const teamApi = new CRUD("api/company/affilations");

    const [isOpen, setIsOpen] = useState(false);

    const [formData, setFormData] = useState<CreateAffiliationDto>(
        {
            image: data.image,
            link: data.link,
            translations: [
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const result = await teamApi.update(formData, affilationId);
