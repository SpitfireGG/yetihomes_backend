...
import { useGetCompanyAffilationsData } from "@/hooks/useTankstack-query"
import TableSkeleton from "../common/table-skeleton"
import { AddCompanyAffilationsDialog } from "../dialog/company-affilations/add-affilations"
import { EditCompanyAffilationsDialog } from "../dialog/company-affilations/edit-affilations"
import Link from "next/link"


export default function CompanyAffilationsTable() {

    const [searchQuery, setSearchQuery] = useState("")
    const [sortBy, setSortBy] = useState("name-asc")
    const { data, isLoading } = useGetCompanyAffilationsData()
        ...
        <div className="space-y-6 p-4 border rounded-md mt-2">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div className="space-y-2">
                    <h1 className="text-2xl font-bold tracking-tight">Company Affilations List</h1>
                    <p className="text-muted-foreground">Manage your csi</p>
                </div>
                <AddCompanyAffilationsDialog />
            </div>
                                    ...
                                    <TableCell className="text-right">
                                        <DropdownMenu>
                                            ...
                                            <DropdownMenuContent align="end">
                                                ...
                                                <DropdownMenuItem>
                                                    <Link href={`/company/affilations/content/${highlight.id}/`} className="flex w-full">
                                                        Add languages
                                                    </Link>
                                                </DropdownMenuItem>
                                                <EditCompanyAffilationsDialog
                                                    affilationId={highlight.id}
                                                    data={
                                                        {
                                                            link: highlight.link,
                                                ...
                                                />
                                                <DeleteDialog id={highlight.id} apiUrl={`api/company/affilations`} />
                                            </DropdownMenuContent>
                                        </DropdownMenu>
                                    </TableCell>
                                </TableRow>
