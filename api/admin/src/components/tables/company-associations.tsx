...
import { useGetCompanyAssociationsData } from "@/hooks/useTankstack-query"
import TableSkeleton from "../common/table-skeleton"
import { PreviewImage } from "../common/image"
import { AddCompanyAssociationsDialog } from "../dialog/company-associations/add-associations"
import { EditCompanyAssociationsDialog } from "../dialog/company-associations/edit-associations"


export default function CompanyAssociationsTable() {
    const [searchQuery, setSearchQuery] = useState("")
    const [sortBy, setSortBy] = useState("name-asc")
    const { data, isLoading } = useGetCompanyAssociationsData()
        ...
        <div className="space-y-6 p-4 border rounded-md mt-2">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div className="space-y-2">
                    <h1 className="text-2xl font-bold tracking-tight">Company Associations</h1>
                    <p className="text-muted-foreground">Manage your associations</p>
                </div>
                <AddCompanyAssociationsDialog />
            </div>
                        ...
                        {paginatedCustomers.length > 0 ? (
                            paginatedCustomers.map((highlight, indx) => (
                                <TableRow key={highlight.id}>
                                    ...
                                    <TableCell>
                                        <a href={highlight.link} target="_blank" className="hover:underline hover:text-primary">
                                            {highlight.link}
                                        </a>
                                    </TableCell>
