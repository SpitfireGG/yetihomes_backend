...
import { useGetCompanyCSIById, useLanguages } from '@/hooks/useTankstack-query';
...
const EditCsi = ({ id: propId }: Props) => {
  const params = useParams();
  const id = ((params.id as string) || propId)!;
  const trips = new CRUD(`api/company/csis`);
  const { data: languages } = useLanguages();
  const { data } = useGetCompanyCSIById(id);
  const csiDetail = data?.data;
  ...
  const defaultTranslation = csiDetail?.translations.find(
    (t) => t.language_id === englishLanguage?.id,
  );
  ...
  useEffect(() => {
    ...
    setFormData({
      image: csiDetail.image_path ?? '',
      slug: csiDetail.slug,
      translations:
        csiDetail.translations?.map((t) => ({
          language_id: englishLanguage?.id ?? t.language_id,
          title: defaultTranslation?.title ?? t.title,
          description: defaultTranslation?.description ?? t.description,
          content: defaultTranslation?.content ?? t.content,
        })) ?? [],
    });
  }, [csiDetail]);
  ...
  const handelSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    try {
      const result = await trips.update(formData, id);
  ...
  return (
    <>
      <h1 className="text-2xl font-bold tracking-tight mt-4">Edit CSI</h1>
      <form onSubmit={handelSubmit} className="py-4">
        <div className="py-4 grid grid-cols-2 gap-6 w-full">
          ...
          <div className="col-span-2 grid md:grid-cols-2 gap-4">
            ...
            <div className="space-y-3">
              <Label className=" capitalize" htmlFor="tabs-demo-name">
        ...
        <div className=" text-right">
          <Button type="submit" className="text-right min-w-2xs">
            Create
          </Button>
        </div>
      </form>
