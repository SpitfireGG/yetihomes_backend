...
import { CompanyAchivementsTranslationForm } from '@/@types/company/achivements'
import TinymceEditor from '@/components/common/TinymceEditor'
import { CompanyCSITranslationFormData } from '@/@types/company/csi'
...
const AddContentForm = ({ destinationId, tripId }: Props) => {
    ...
    const destinationCrud = new CRUD(`api/company/csis`)
    const { data } = useGetCompanyCSIById(destinationId)
    const destination = data?.data
    const router = useRouter()
    const { data: lang, isLoading } = useLanguages()
    const languages = lang?.data ?? []

    const [formData, setFormData] = useState<CompanyCSITranslationFormData>({
        slug: '',
        translations: [],
    })
    ...
    const updateTranslation = (
        index: number,
        field: 'language_id' | 'title' | 'description' | 'content',
        value: string
    ) => {
        setFormData(prev => {
                    ...
                    return (
                        ...
                        >
                            ...
                            <div className='space-y-3'>
                                <Label>Content</Label>
                                <TinymceEditor value={translation.content} onChange={(value) => updateTranslation(index, 'content', value)} />
                            </div>
                        </div>
                    )
                })}
