...
const getPropertyIdFromLocation = (): string | undefined => {
  ...
  const editIndex = segments.findIndex(
    (segment, index) =>
      segment === 'properties' && segments[index + 1] === 'edit',
  );
...
export default function EditPropertyPage({ id: propId }: { id?: string }) {
  ...
  const propertyId =
    ...
    (paramId && paramId !== 'placeholder' ? paramId : undefined);

  const [isLoading, setIsLoading] = useState(false);
  const [isFetching, setIsFetching] = useState(Boolean(propertyId));
  ...
  useEffect(() => {
    const fetchProperty = async () => {
      try {
        const res = await propertyCrud.authFetch(
        ...
        setSelectedAmenities(
          property.propertyAmenities?.map((pa: any) => pa.amenityId) || [],
        );
        setServicesNearby(
          property.servicesNearby?.map((s: any) => ({
        ...
        setFormData({
          ...
          houseDetails: {
            ...
            furnishingStatus:
              property.houseDetails?.furnishingStatus || 'SEMI_FURNISHED',
            buildYear: property.houseDetails?.buildYear || 0,
            facingDirection: property.houseDetails?.facingDirection || 'EAST',
            roadType: property.houseDetails?.roadType || 'PITCHED',
            roadSize: property.houseDetails?.roadSize || 0,
          },
          apartmentDetails: {
            ...
            furnishingStatus:
              property.apartmentDetails?.furnishingStatus || 'SEMI_FURNISHED',
            facingDirection:
              property.apartmentDetails?.facingDirection || 'EAST',
            roadType: property.apartmentDetails?.roadType || 'PITCHED',
            roadSize: property.apartmentDetails?.roadSize || 0,
          },
          landDetails: {
            ...
            roadType: property.landDetails?.roadType || 'PITCHED',
  ...
  if (!propertyId) {
    return (
      <div className="flex items-center justify-center py-20 text-sm text-muted-foreground">
        Property ID is missing from the edit URL.
      </div>
    );
  }
          ...
          {propertyType === 'LAND' && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="flex items-center space-x-2 pt-6">
                <Switch
                  checked={formData.landDetails?.isCornerPlot}
                  onCheckedChange={(checked) =>
                    setFormData({
                      ...formData,
                      landDetails: {
