...
export default function EditBlogPage({ id: propId }: { id?: string }) {
  const router = useRouter();
  const params = useParams();
  const blogId = (params.id as string) || propId;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok || !result.success) {
        throw new Error(result.message || 'Failed to update blog article');
      }
