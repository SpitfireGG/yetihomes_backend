...
export const useUpdateTeam = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ data, id }: { data: unknown; id: string }) =>
      teams.update(data, id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['teams'] });
    },
  });
...
export const useGetCompanyTermsData = () =>
  useQuery({
...
export const useGetCompanyTermsById = (id: string) =>
  useQuery({
...
export const useGetCompanyPrivacyById = (id: string) =>
  useQuery({
...
export const useGetCompanySustainability = () =>
  useQuery({
...
export const useGetCompanySustainabilityById = (id: string) =>
  useQuery({
...
export const useGetCompanyCookies = () =>
  useQuery({
...
export const useGetCompanyCookiesById = (id: string) =>
  useQuery({
...
export const useGetCompanyCSIData = () =>
  useQuery({
    queryKey: ['companyCSI'],
...
export const useGetCompanyCSIById = (id: string) =>
  useQuery({
...
export const useGetCompanyAchievementsData = () =>
  useQuery({
...
export const useGetCompanyAchievementsById = (id: string) =>
  useQuery({
...
export const useGetCompanyAffilationsData = () =>
  useQuery({
...
export const useGetCompanyAffilationsById = (id: string) =>
  useQuery({
...
export const useGetCompanyGuidesData = () =>
  useQuery({
...
export const useGetCompanyGuidesById = (id: string) =>
  useQuery({
...
export const useGetCompanyLegalDocsData = () =>
  useQuery({
...
export const useGetCompanyLegalDocsById = (id: string) =>
  useQuery({
...
export const useGetCompanyContactInfoData = () =>
  useQuery({
...
export const useGetCompanyContactInfoById = (id: string) =>
  useQuery({
...
export const useGetCompanyAssociationsData = () =>
  useQuery({
...
export const useGetCompanyAssociationsById = (id: string) =>
  useQuery({
...
export const useGetContentsById = (id: string) =>
  useQuery({
...
const supportTickets = new CRUD('api/contact/tickets');

export const useGetCompanySocialMedia = () =>
  useQuery<{ data: any[] }>({
...
export const useSupportTickets = () =>
  useQuery<{ data: any[] }>({
    ...
    queryFn: () => supportTickets.getData(),
    staleTime: 5 * 60 * 1000,
  });
...
export const useDashboard = (range: string = '30d') => {
  return useQuery<DashboardData>({
    ...
    queryFn: async () => {
      ...
      const publishedListings = propertiesData.filter(
        (p: any) => p.status === 'PUBLISHED',
      ).length;
      const draftListings = propertiesData.filter(
        (p: any) => p.status === 'DRAFT',
      ).length;
      const soldListings = propertiesData.filter(
        (p: any) => p.status === 'SOLD',
      ).length;
      const rentedListings = propertiesData.filter(
        (p: any) => p.status === 'RENTED',
      ).length;
      const archivedListings = propertiesData.filter(
        (p: any) => p.status === 'ARCHIVED',
      ).length;

      const houses = propertiesData.filter(
        (p: any) => p.propertyType === 'HOUSE',
      ).length;
      const apartments = propertiesData.filter(
        (p: any) => p.propertyType === 'APARTMENT',
      ).length;
      const lands = propertiesData.filter(
        (p: any) => p.propertyType === 'LAND',
      ).length;

      const newEnquiries = inquiriesData.filter(
        (i: any) => i.status === 'NEW',
      ).length;
      ...
      const currentListings = propertiesData.filter(
      ...
      const totalListingsDelta =
        prevListings > 0
          ? ((currentListings - prevListings) / prevListings) * 100
          : 0;

      const currentEnquiries = inquiriesData.filter(
        (i: any) => new Date(i.createdAt) >= periodStart,
      ).length;
      const prevEnquiries = inquiriesData.filter((i: any) => {
        const d = new Date(i.createdAt);
        return d >= prevPeriodStart && d < periodStart;
      }).length;
      const newEnquiriesDelta =
        prevEnquiries > 0
          ? ((currentEnquiries - prevEnquiries) / prevEnquiries) * 100
          : 0;

      const currentDeals = propertiesData.filter((p: any) => {
        const d = new Date(p.updatedAt);
        return (
          d >= periodStart && (p.status === 'SOLD' || p.status === 'RENTED')
        );
      }).length;
      ...
      const closedDealsDelta =
        prevDeals > 0 ? ((currentDeals - prevDeals) / prevDeals) * 100 : 0;

      const currentGmv = propertiesData
        .filter(
          (p: any) =>
            (p.status === 'SOLD' || p.status === 'RENTED') &&
            new Date(p.updatedAt) >= periodStart &&
            p.priceAmount,
        )
        .reduce((sum: number, p: any) => sum + Number(p.priceAmount), 0);
      ...
      const recentEnquiries = inquiriesData
        ...
        .map((inquiry: any) => ({
          ...
          propertyLocation:
            inquiry.property?.locationText || inquiry.property?.city || 'N/A',
      ...
      const topListings = [...propertiesData]
        .map((property: any) => {
          ...
          const propertyEnquiries = inquiriesData.filter(
